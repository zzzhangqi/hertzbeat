 ...
 * @date 2022/9/9
 */
@ProtocolType(name = "http")
public class HttpProtocolImpl implements ProtocolTypeDiptach {
    @Override
    public Map<String, Object> dispatch(Metrics request) {
        JacksonUtil<HttpProtocol> jacksonUtil = new JacksonUtil<>();
        return jacksonUtil.readValue(request.getHttp());
    }
}
