 ...
 * @date 2022/9/9
 */
@ProtocolType(name = "ssh")
public class SshProtocolImpl implements ProtocolTypeDiptach {
    @Override
    public Map<String, Object> dispatch(Metrics request) {
        JacksonUtil<SshProtocol> jacksonUtil = new JacksonUtil<>();
        return jacksonUtil.readValue(request.getSsh());
    }
}
